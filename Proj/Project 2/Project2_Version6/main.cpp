 ...
 * File:   main.cpp
 * Author: Leo Okdemir
 * Created on February 10, 2024, 6:35 PM
...
// Arrays and Vectors
void disArry(const int arr[], int size); //Display Array
void disVect(const vector<int>& vec); //Display Vector
void iniArry(int arr[], int size); //Initialize Array
void iniVect(vector<int>& vec, int size); //Initialize Vector
void ArryVal(int arr[], int size); //Pass Array by Value
bool ArryRef(int arr[], int size); //Pass Array by reference
bool VectRef(vector<int>& vec); //Pass Vector by Reference
...
void slcSort(int arr[], int size); //Selection Sort
bool linSrch(const int arr[], int size, int trgt);//Linear Search
bool binSrch(const int arr[], int size, int trgt); //Binary Search
...
int main() {
    ...
    iniArry(intArry, ARSIZE);
    iniVect(intVect, ARSIZE);

    // Call functions to demonstrate array and vector operations
    disArry(intArry, ARSIZE);
    disVect(intVect);

    ArryVal(intArry, ARSIZE);

    if (ArryRef(intArry, ARSIZE)) {
        cout << "Array changed" << endl;
        disArry(intArry, ARSIZE);
    } else {
        cout << "Array didnt change" << endl;
    }

    if (VectRef(intVect)) {
        cout << "Vector changed" << endl;
        disVect(intVect);
    } else {
        cout << "Vector didnt change" << endl;
    }

    cout << "Sum of array elements: " << sumArry(intArry, ARSIZE) << endl;
    cout << "Sum of vector elements: " << sumVect(intVect) << endl;

    bblSort(intArry, ARSIZE);
    slcSort(intArry, ARSIZE);
    ...
    do {
        ...
        if (strtBtl == 'b') {
            ...
            while (contWar && plWar(btlNum, strtBtl, p1Crd, p1Suit, p2Crd, p2Suit, p1Value, p2Value, out, CARDS, SUITS, p1Wins, p2Wins)) {
                contWar = !plBttl(btlNum, strtBtl, p1Crd, p1Suit, p2Crd, p2Suit, p1Value, p2Value, out, CARDS, SUITS, p1Wins, p2Wins);
            }
        }
...
bool plWar(int& btlNum, char& strtBtl, char& p1Crd, char& p1Suit, char& p2Crd, char& p2Suit, int& p1Value, int& p2Value, ofstream& out, const string& CARDS, const string& SUITS, int& p1Wins, int& p2Wins) {
    ...
    vector<int> p1DnVal(3);
    vector<int> p2DnVal(3);

    for (int i = 0; i < 3; i++) {
        disCard(p1Down[i], p1Suit, p2Down[i], p2Suit, p1DnVal[i], p2DnVal[i], CARDS, SUITS);
    }
...
void disArry(const int arr[], int size) {
    out << "Array elements: ";
...
void disVect(const vector<int>& vec) {
    out << "Vector elements : ";
    for (int num : vec) {
        out << num << " ";
    }
...
bool ArryRef(int arr[], int size) {
    out << "Modifying array by reference : " << endl;
    for (int i = 0; i < size; ++i) {
        arr[i] *= 2;
    }
    return true;
...
bool VectRef(vector<int>& vec) {
    out << "Modifying vector by reference : " << endl;
    for (int& num : vec) {
        num *= 2;
    }
    return true;
...
bool linSrch(const int arr[], int size, int trgt) {
    ...
    out << "Element " << trgt << " not found in the array." << endl;
    return false;
}

bool binSrch(const int arr[], int size, int trgt) {
    ...
    while (low <= high) {
        ...
        if (arr[mid] == trgt) {
            cout << trgt << " found at index " << mid << endl;
            return true;
