 ...
 * File:   main.cpp
 * Author: Leo Okdemir
 * Created on February 8, 2024, 5:45 PM
...
bool valInput(char& input); // Validate Input
...
int main() {
    ...
    do {
        bool continueWar = true;  // Reset the war flag for each battle
            ...
            while (contWar && plWar(btlNum, strtBtl, p1Crd, p1Suit, p2Crd, p2Suit, p1Value, p2Value, out, CARDS, SUITS, p1Wins, p2Wins))
{
    contWar = plBttl(btlNum, strtBtl, p1Crd, p1Suit, p2Crd, p2Suit, p1Value, p2Value, out, CARDS, SUITS, p1Wins, p2Wins);
        ...
        strtBtl = 'b'; // Reset strtBtl to 'b' for the next battle
        btlNum++;
        cout << endl;

        if ((p1Wins + p2Wins) < 52) {
            continueGame = true;  // Set the flag to continue the loop
        } else {
            if (p1Wins > p2Wins) {
